...
Run the **entire canonical workflow** for one JIRA task. User typically invokes: `/ae-autopilot:launch TSK-123`.
...
2. **Run in order**  
   ...
   - **Pull:** `/ae-autopilot:pull` behavior — fetch JIRA task, sync release branch, create branch `TSK-123`. Artifacts: `.autopilot/task.json`, `.autopilot/state.json`.
   - **Plan:** `/ae-autopilot:plan` behavior — classify (L0–L3), generate plan if L2/L3. Artifacts: `.autopilot/classification.json`, `plan.yml` if L2/L3.
   - **Execute:** `/ae-autopilot:execute-plan` behavior — execute work; for L3 use phases with confirmation between. Update state and commits.
   - **Review:** `/ae-autopilot:review` behavior — dbt build/tests, lint, contracts. If failed, stop and report.
   - **PR:** `/ae-autopilot:pr` behavior — open PR with template; never merge.
...
4. **Optional flags (from spec)**  
   ...
   If user says `--stop-after=plan`: stop after `/ae-autopilot:plan`, do not execute.  
